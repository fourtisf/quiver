...
const ROBINFUN_API =
  process.env.NEXT_PUBLIC_ROBINFUN_API ?? "https://robinfun.io/api/v1";
...
function parseToken(raw: unknown): TokenInfo | null {
  ...
  return { address, symbol, name, decimals: 18, logoURI, robinfun: true };
}

export async function fetchRobinfunTokens(): Promise<TokenInfo[]> {
  for (const path of ["/tokens?limit=100", "/tokens"]) {
    try {
      const res = await fetch(`${ROBINFUN_API}${path}`, { headers: { accept: "application/json" } });
      if (!res.ok) continue;
